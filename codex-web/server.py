...
CODEX_WORKSPACE = "/home/exedev/codex-workspace"
SYNC_SCRIPT = "/home/exedev/sync-codex-workspace.sh"
...
async def upload_files(files: List[UploadFile] = File(...)):
    """Upload text files, PDFs, and images. Images are auto-described via GPT-4o vision."""
    results = []
    for f in files:
        ext = Path(f.filename).suffix.lower()
        if ext not in (".md", ".txt", ".css", ".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg", ".pdf"):
...
async def skills_library_page():
    return FileResponse(Path(__file__).parent / "static" / "skills-library.html")
...
async def codex_ws(websocket: WebSocket):
    ...
    async def process_queue():
        while True:
            ...
            # Sync project -> workspace (lean copy for Codex context)
            ...
            try:
                ...
                # Sync changes back: workspace -> project
                rsync_proc = await asyncio.create_subprocess_exec(
                    "rsync", "-a", "--existing", "--exclude", ".git",
                    CODEX_WORKSPACE + "/", PROJECT_DIR + "/",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                )
                await rsync_proc.wait()
                # Also copy any NEW files codex created
                new_files_proc = await asyncio.create_subprocess_exec(
                    "bash", "-c",
                    f'cd "{CODEX_WORKSPACE}" && find . -type f -newer "{SYNC_SCRIPT}" '
                    f'-not -path "./.git/*" -not -name "*.pyc" '
                    f'| while read f; do mkdir -p "{PROJECT_DIR}/$(dirname "$f")"; '
                    f'cp "$f" "{PROJECT_DIR}/$f"; done',
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                )
                await new_files_proc.wait()
