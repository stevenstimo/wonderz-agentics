          ...
          {loading ? (
            ...
            <div className="grid gap-4">
              {documents.map((doc) => (
                <div
                  key={doc.document_id}
                  className="flex items-start gap-3 p-4 rounded-xl border border-slate-200 bg-white hover:border-indigo-300 hover:shadow-md transition-all"
                >
                  <Link to={`/knowledge/${doc.document_id}`} className="min-w-0 flex-1 block">
                  ...
                  {doc.status !== 'archived' && (
                    <Link
                      to={`/knowledge/${doc.document_id}/edit`}
                      onClick={(e) => e.stopPropagation()}
                      className="flex-shrink-0 p-2 rounded-lg text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 transition-colors"
                      title="Bewerken"
                    >
