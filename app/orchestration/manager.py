    ...
    def determine_next_step(self, current_step: str, context: SharedContext) -> Optional[str]:
        ...
        if current_step == "reviewer_agent":
            ...
            if isinstance(review_out, dict):
                status = (review_out.get("status") or "").upper()

            # Workflow contract: approved review transitions to JOB_READY.
            if status == "APPROVED":
                return "JOB_READY"

            # If reviewer requests changes, iterate once more through copy then review.
            ...
            # Assumption-based: unknown reviewer output is treated as retryable once.
            retries = context.metadata.get("retries", 0)
            if retries < 1:
                context.metadata["retries"] = retries + 1
                return "copy_agent"
        ...
        try:
            ...
            while steps < max_steps:
                ...
                if next_agent is None:
                    # Assumption-based: undefined transition means pause for manual approval.
                ...
                # Emit subtask milestones so UI can show granular progress within a major task.
                await self.write_job_step(
                    conn,
                    job_id,
                    f"{next_agent}::prepare",
                    next_agent,
                    "success",
                    output={"info": "agent input prepared"}
                )
                await self.write_job_step(
                    conn,
                    job_id,
                    f"{next_agent}::execute",
                    next_agent,
                    "in_progress",
                    output={"info": "agent execution started"}
                )
                ...
                await self.write_job_step(
                    ...
                    next_agent,
                    "success",
                    output={"info": "agent execution completed"}
                )
                ...
                await self.write_job_step(
                    conn,
                    job_id,
                    f"{next_agent}::finalize",
                    next_agent,
                    "success",
                    output={"info": "agent output persisted"}
                )
                ...
                if isinstance(result, dict) and result.get("artifacts"):
                    for art in result.get("artifacts"):
                        try:
                            ...
                            # Assumption-based: artifact persistence errors must not block workflow progression.
                            pass
