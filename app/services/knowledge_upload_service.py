...
async def run_embedding_task(pool, document_id: str) -> None:
    ...
    if not rows:
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE knowledge_documents SET embedding_status = 'failed' WHERE document_id = $1::uuid",
                document_id,
            )
...
async def reindex_document(
    ...
    *,
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "UPDATE knowledge_documents SET embedding_status = 'processing' WHERE document_id = $1::uuid",
            document_id,
        )
        ...
        try:
            ...
            await conn.execute(
                "UPDATE knowledge_documents SET embedding_status = 'complete' WHERE document_id = $1::uuid",
                document_id,
            )
        except Exception as exc:
