# CURSOR — Docs Structuur Optie B + Master Index
**Datum:** 18 maart 2026
**Type:** Structuurwijziging — geen code, geen migrations, geen DB

---

## Context

Dit document reorganiseert de `docs/` map naar een schaalbare structuur (Optie B).
Alle bestaande inhoud blijft behouden — alleen verplaatsen, hernoemen en uitbreiden.
...
- Geen migrations
- Geen DB wijzigingen
- Geen frontend of backend aanpassingen
- Alleen bestanden aanmaken, verplaatsen en updaten in `docs/` en `.cursorrules`

---

## Fase 1 — Mapstructuur aanmaken

Maak de volgende mappenstructuur aan in de repo:
...
Commando:
```bash
mkdir -p docs/framework docs/cursor docs/specs
```

---

## Fase 2 — Bestaande bestanden verplaatsen

Verplaats de bestaande docs naar de juiste map:

```bash
# Framework spec → docs/framework/
mv docs/260317_crew_intelligent_agent_framework.md \
   docs/framework/260317_crew_intelligent_agent_framework.md

# Bestaande bewaker → docs/cursor/
mv docs/260317_CURSOR_crew_intelligent_phased_implementation.md \
   docs/cursor/01_phased_implementation.md
```

Het nieuwe bestand `260318_CURSOR_dashboard_newbies_navigation.md`
(als het al in docs/ staat) verplaatsen naar:
```bash
mv docs/260318_CURSOR_dashboard_newbies_navigation.md \
   docs/cursor/02_dashboard_newbies_navigation.md
```

Als het bestand nog niet in de repo staat: maak het aan als
`docs/cursor/02_dashboard_newbies_navigation.md` met de inhoud
uit sectie "Fase 5-7 inhoud" hieronder.

**Stop hier. Verifieer dat alle bestanden op de juiste plek staan.
Rapporteer de bestandsstructuur. Wacht op bevestiging.**

---

## Fase 3 — Master Index aanmaken

Maak `docs/cursor/00_master_index.md` aan met de volgende inhoud:
...
**Stop hier. Rapporteer dat de master index is aangemaakt.
Wacht op bevestiging.**

---

## Fase 4 — .cursorrules updaten

Vervang de bestaande Crew Intelligent regel in `.cursorrules`:

**Oud:**
```
## Crew Intelligent / Agent Lifecycle
Bij taken over Agent Lifecycle, NewCrewMember, hired_agents,
persona batch of Training Workflow: lees eerst
docs/260317_CURSOR_crew_intelligent_phased_implementation.md
en volg de fasering exact.
```
...
hired_agents, newbies, persona batch, Training Workflow,
Dashboard, navigatie of C-suite architectuur:

1. Lees eerst docs/cursor/00_master_index.md volledig
2. Bepaal welke fase actief is
3. Lees het bijbehorende fase-document uit docs/cursor/
4. Volg de fasering exact

NOOIT direct een fase-document openen zonder eerst
de master index te lezen.
```

**Stop hier. Rapporteer dat .cursorrules is bijgewerkt.
Wacht op bevestiging.**
...
Controleer de volledige structuur:
...
Verwachte output:
```
docs/cursor/00_master_index.md
docs/cursor/01_phased_implementation.md
docs/cursor/02_dashboard_newbies_navigation.md
docs/framework/260317_crew_intelligent_agent_framework.md
```

En `.cursorrules` bevat de nieuwe Crew Intelligent sectie.

**Rapporteer de volledige bestandslijst.**
...
cd ~/wonderz-agentics
git add -A
git commit -m "docs: restructure to Option B — master index, fase-documents, cursor rules"
git push
```

Geen backend restart nodig — alleen docs en .cursorrules gewijzigd.

---

## Fase 5-7 inhoud (voor 02_dashboard_newbies_navigation.md)

Als het bestand `02_dashboard_newbies_navigation.md` nog niet bestaat,
gebruik dan de inhoud uit `260318_CURSOR_dashboard_newbies_navigation.md`
(het bestand dat eerder is aangemaakt). Kopieer de volledige inhoud.

Als dat bestand ook niet beschikbaar is, rapporteer dit dan aan de
gebruiker — niet zelf invullen.

---

*Na deze structuurwijziging is docs/cursor/00_master_index.md
altijd het eerste bestand dat Cursor leest bij Crew Intelligent taken.*
