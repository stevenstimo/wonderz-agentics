...
**ARCHITECTUURNOTITIE (v1.1):** De platformarchitectuur gebruikt een volledige C-suite structuur. COO = operationeel orchestrator (intake, delegatie, pipeline). CEO = strategisch monitorniveau (ziet geen individuele jobs). Alle huidige orchestrator-agents in hired_agents zijn COO-niveau. Zie framework sectie 1 voor volledige C-suite taakverdeling en rapportagelijnen.
...
| `category` | `type` (worker/talent/orchestrator) | ADD COLUMN + CHECK constraint. Orchestrators zijn COO-niveau (operationeel), niet CEO-niveau (strategisch). |
...
| `type` | Radio (worker / talent / orchestrator). Orchestrators = COO-niveau in de pipeline, niet CEO. | Ja |
| `goal` | Text input | Ja |
...
- `type` is één van: `worker`, `talent`, `orchestrator` (orchestrators functioneren als COO in de pipeline)
...
Subfase 4a — hired_agents records aanmaken
  ...
  → Orchestrator-agents die hier worden aangemaakt functioneren als COO in de pipeline (intake, delegatie)
...
Subfase 4b — development_points aanmaken
  ...
  → Geldt ook voor orchestrator/COO-agents
...
Subfase 4d — agents activeren
  ...
  → Orchestrator-agents functioneren na activatie als COO in de pipeline
...
Voorbeelden:
  ...
  agent:orchestrator:jeanne-darc-001   (COO-niveau in pipeline)
```
