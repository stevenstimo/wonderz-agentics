# 260328_CURSOR_role_taxonomy_presets
...
Wonderz-Agentics, FastAPI + asyncpg backend, Supabase als database.

De Role Taxonomy is volledig uitgewerkt in `docs/260317_CURSOR_crew_intelligent_agent_framework.md`. 49 personas zijn ingedeeld in drie typen (Worker, Talent, CEO/Orchestrator) met bijbehorende rollen, readiness scores, guardrails en rol-templates.

Deze data is beschikbaar als document maar bestaat nog niet als opvraagbare DB tabel. De CEO (Donna) en de HiringHall moeten kunnen zien welke rollen beschikbaar zijn en welke personas daarvoor het meest geschikt zijn.

**Doel:** Sla de Role Taxonomy op als `agent_role_presets` tabel in Supabase zodat:
1. De HiringHall de beschikbare rollen en bijbehorende personas kan tonen
2. De CEO bij resource-check kan matchen op rol-type
3. NewCrewMember formulier kan suggereren welke persona bij een rol past
...
```bash
# 1. Check of agent_role_presets al bestaat
grep -rn "agent_role_presets\|role_presets\|role_taxonomy" app/ --include="*.py" | head -10

# 2. Check bestaande rollen in hired_agents
# (via Supabase MCP of terminal)
# SELECT DISTINCT role, type FROM hired_agents ORDER BY type, role;

# 3. Check job_type_presets voor referentie-patroon
grep -n "job_type_presets" app/routes/ -r --include="*.py" | head -5

# 4. Check of er al een roles endpoint bestaat
grep -rn "roles\|/roles" app/routes/ --include="*.py" | head -10
```

Rapporteer de output. Stop na pre-flight en wacht op bevestiging.

---

## Fase 1 — Database: agent_role_presets tabel

### Migratie via Supabase MCP

```sql
CREATE TABLE IF NOT EXISTS agent_role_presets (
    role_id         TEXT PRIMARY KEY,           -- bijv. 'copywriter', 'seo-specialist'
    role_label      TEXT NOT NULL,              -- bijv. 'Copywriter', 'SEO Research'
    agent_type      TEXT NOT NULL               -- 'worker', 'talent', 'orchestrator'
                    CHECK (agent_type IN ('worker', 'talent', 'orchestrator')),
    description     TEXT NOT NULL,              -- korte omschrijving van de rol
    tool_whitelist  TEXT[] DEFAULT '{}',        -- standaard tools voor deze rol
    output_format   JSONB DEFAULT '{}',         -- standaard output format
    guardrails      JSONB DEFAULT '{}',         -- standaard guardrails
    model_config    JSONB DEFAULT '{}',         -- aanbevolen model config
    suggested_personas JSONB DEFAULT '[]',      -- [{persona, score, development_priority}]
...
### Seed data — Worker rollen (11)

```sql
INSERT INTO agent_role_presets (role_id, role_label, agent_type, description, tool_whitelist, output_format, guardrails, model_config, suggested_personas) VALUES
...
### Seed data — Orchestrator rollen (2)
...
Voer de SQL uit via Supabase MCP. Rapporteer hoeveel rijen zijn ingevoegd.

Stop na fase 1 en wacht op bevestiging.

---

## Fase 2 — Backend: roles endpoint

### Nieuw bestand `app/routes/roles.py`
...
async def list_role_presets(
    ...
    """Haal alle beschikbare rol-presets op. Optioneel gefilterd op agent_type."""
    pool = await get_db()
    async with pool.acquire() as conn:
        if agent_type:
            rows = await conn.fetch(
                "SELECT * FROM agent_role_presets WHERE is_active = true AND agent_type = $1 ORDER BY agent_type, role_label",
                agent_type,
            )
        else:
            rows = await conn.fetch(
                "SELECT * FROM agent_role_presets WHERE is_active = true ORDER BY agent_type, role_label"
            )
...
async def get_role_preset(
    ...
    """Haal één rol-preset op inclusief suggested_personas."""
    pool = await get_db()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT * FROM agent_role_presets WHERE role_id = $1",
            role_id,
        )
    if not row:
...
curl -s http://localhost:8090/api/roles | python3 -m json.tool | head -20
```

Stop na fase 2 en wacht op bevestiging.

---

## Fase 3 — Frontend: rollen zichtbaar in NewCrewMember

In `web_ui/frontend/src/NewCrewMember.jsx`:

1. Laad `GET /api/roles` bij mount.
2. Voeg een rol-selector toe bovenaan het formulier: dropdown met alle beschikbare rollen gesorteerd op type.
3. Als een rol wordt geselecteerd: vul automatisch voor:
   - `tool_whitelist` uit `role.tool_whitelist`
   - `output_format` uit `role.output_format`
   - Toon de `suggested_personas` als aanbeveling: "Aanbevolen persona's voor deze rol: X, Y, Z"
4. Gebruiker kan de automatisch ingevulde velden nog steeds overschrijven.

### Acceptatiecriteria fase 3

- Rol-selector zichtbaar in NewCrewMember
- Selecteren van rol vult tool_whitelist en output_format voor
- Suggested personas worden getoond als niet-bindende suggestie
...
- Geen wijzigingen aan bestaande agent-routes
...
```bash
# Fase 2
git add app/routes/roles.py app/main.py
git commit -m "feat: agent_role_presets tabel en roles endpoint"

# Fase 3
git add web_ui/frontend/src/NewCrewMember.jsx
git commit -m "feat: rol-selector met auto-fill en suggested personas in NewCrewMember"
